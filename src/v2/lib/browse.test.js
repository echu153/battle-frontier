...
import {
  ...
  defaultRuneFilter, filterRunes, sortRunes,
} from './browse.js'
...
test('ルーンは色と特殊能力の有無で絞り込める', () => {
  assert.deepEqual(filterRunes(ESS, defaultRuneFilter).map(e => e.id), [1, 2, 3, 4])
  assert.deepEqual(filterRunes(ESS, { color:'blue' }).map(e => e.id), [2, 4])
  assert.deepEqual(filterRunes(ESS, { ability:'あり' }).map(e => e.id), [1, 4])
  assert.deepEqual(filterRunes(ESS, { ability:'なし' }).map(e => e.id), [2, 3])
  assert.deepEqual(filterRunes(ESS, { color:'blue', ability:'あり' }).map(e => e.id), [4])
})

test('ルーンは合計値・名前・色で並べ替えられる', () => {
  assert.deepEqual(sortRunes(ESS, 'power', false).map(e => e.id), [3, 4, 1, 2])
  assert.deepEqual(sortRunes(ESS, 'power', true).map(e => e.id), [2, 1, 4, 3])
  // 色は緋→蒼→翠の順（COLORS の並び）
  assert.deepEqual(sortRunes(ESS, 'color', true).map(e => e.color), ['red', 'blue', 'blue', 'green'])
})

test('同じ値のときは合計値の大きい順→id順でそろう（並びがちらつかない）', () => {
  const same = [ess(9, 'blue', { vit: 1.0 }), ess(2, 'blue', { vit: 1.0 }), ess(5, 'blue', { vit: 4.0 })]
  assert.deepEqual(sortRunes(same, 'color', false).map(e => e.id), [5, 2, 9])
})

test('ページ送りはルーンでも装備と同じ関数を使う', () => {
