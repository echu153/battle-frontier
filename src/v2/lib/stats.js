...
//   実際のステ更新の権威はサーバー（supabase_v2_core.sql の v2_apply_exp）。
//   数式を変えるときは必ず両方を同時に直すこと（片方だけだと表示と実値がズレる）。
//   SQL側は int[] の並びで抽選するため、STAT_KEYS の順序＝SQLの配列の並び。
// ============================================================

// 抽選の並び。SQL側 v_gain/v_unit 配列の 1〜8 と一致させること
...
// ===== 成長の定数（あるけみすと準拠。調整するときはSQL側の同名定数も直す） =====
export const MAX_LV = 100        // LV上限。到達後はEXPが入らない（転生は未実装）
export const EXP_PER_LV = 60     // LVを1上げるのに必要なEXP
...
// 次のLVまでに必要なEXP。LV上限なら0（＝もう溜まらない）
export const expToNext = (lv) => (lv >= MAX_LV ? 0 : EXP_PER_LV)

// 戦闘力。HPは8、MPは3で戦闘力1に換算し、他6ステはそのまま加算する
export const calcPower = (s) =>
  Math.floor(STAT_KEYS.reduce((t, k) => t + (s[k] || 0) / STAT_DEFS[k].unit, 0))

// LVアップ1回分の抽選。8種から均等に ROLLS_PER_LV 回引き、当たったステを unit だけ上げる
export const rollLevelUp = (rng = Math.random) => {
  const gains = emptyGains()
  for (let i = 0; i < ROLLS_PER_LV; i++) {
...
export const applyExp = (state, amount, rng = Math.random) => {
  ...
  if (lv < MAX_LV && amount > 0) {
    exp += amount
    while (lv < MAX_LV && exp >= EXP_PER_LV) {
      exp -= EXP_PER_LV
      lv += 1
    ...
    if (lv >= MAX_LV) exp = 0  // 上限到達＝あふれたEXPは捨てる
  }
