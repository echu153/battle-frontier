  ...
  const toggle = async () => {
    ...
    try {
      if (status === 'on') {
        ...
        setMsg('レイド通知をONにした。毎日21時・22時に知らせてやる。')
      }
    } catch (e) {
      const m = e?.message || ''
      if (m === 'denied') setMsg('通知が許可されていない。端末/ブラウザの設定で通知を許可してくれ。')
      else if (m === 'not_configured') setMsg('通知はまだ準備中だ（サーバー設定待ち）。')
      else if (m === 'unsupported') setMsg('この端末/ブラウザは通知に対応していない。')
      else if (m === 'unauth') setMsg('ログインし直してから試してくれ。')
      else setMsg('うまくいかなかった。時間を置いて、もう一度試してくれ。')
    } finally {
      setBusy(false)
    }
  }
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 'min(360px, 100%)', background: '#000a1c', border: '1px solid #44ddaa', borderRadius: '8px', fontFamily: 'monospace', color: '#cfe', padding: '16px' }}>
        ...
        <p style={{ fontSize: '12px', lineHeight: 1.7, color: '#9bd' }}>
          ONにすると、レイドボスが出現する<b>毎日21時・22時</b>に、アプリを閉じていてもスマホへ通知が届く。
        </p>

        {status === 'unsupported' && (
          <p style={{ fontSize: '12px', color: '#cc9944' }}>この端末/ブラウザは通知に対応していない。</p>
        )}
        {status === 'notconfigured' && (
          <p style={{ fontSize: '12px', color: '#cc9944' }}>通知はまだ準備中だ（サーバー設定待ち）。</p>
        )}
        {status === 'denied' && (
          <p style={{ fontSize: '12px', color: '#cc9944' }}>通知がブロックされている。端末/ブラウザの設定で、このサイトの通知を「許可」にしてから開き直してくれ。</p>
        )}
        ...
        <p style={{ fontSize: '10px', color: '#557', marginTop: '12px', lineHeight: 1.6 }}>
          ※通知は端末ごとの設定だ（スマホとPCで別々）。<br />
          ※iPhoneは「ホーム画面に追加」してアプリとして開いてからでないとONにできない（iOS16.4以降）。
        </p>
      </div>
    </div>
  )
