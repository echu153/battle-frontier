...
const CLASSES = [
  { id:'戦士',    icon:'⚔️',  hp:120, mp:20,  atk:15, def:10, matk:5,  mdef:8,  spd:8,  desc:'高HP・高防御の前衛' },
  { id:'弓使い',  icon:'🏹',  hp:90,  mp:40,  atk:12, def:6,  matk:8,  mdef:6,  spd:15, desc:'素早さ特化のバランス型' },
  { id:'魔法使い',icon:'🔮',  hp:60,  mp:100, atk:5,  def:3,  matk:20, mdef:10, spd:10, desc:'高魔力・強力な魔法攻撃' },
  { id:'僧侶',    icon:'✨',  hp:80,  mp:80,  atk:6,  def:6,  matk:15, mdef:15, spd:9,  desc:'回復・魔法防御が高い' },
]
      ...
      <div style={{ border:'1px solid #0044aa', background:'#001040', padding:'30px', width:'460px' }}>
        ...
        <form onSubmit={handle} style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
          ...
          <div style={{ background:'#000818', border:'1px solid #002244', padding:'10px', fontSize:'11px' }}>
            <div style={{ color:'#88ccff', marginBottom:'6px' }}>{c.desc}</div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'4px', color:'#446688' }}>
              <span>HP: <span style={{color:'#44ff88'}}>{c.hp}</span></span>
              <span>MP: <span style={{color:'#4488ff'}}>{c.mp}</span></span>
              <span>ATK: <span style={{color:'#ffcc00'}}>{c.atk}</span></span>
              <span>DEF: <span style={{color:'#88aaff'}}>{c.def}</span></span>
              <span>MATK: <span style={{color:'#cc44ff'}}>{c.matk}</span></span>
              <span>MDEF: <span style={{color:'#44ccff'}}>{c.mdef}</span></span>
              <span>SPD: <span style={{color:'#ff8844'}}>{c.spd}</span></span>
            </div>
          </div>
          ...
          <button type="submit" disabled={loading}
            style={{ background:'#001840', border:'1px solid #ffcc00', color:'#ffcc00', padding:'10px', cursor:'pointer', fontFamily:'monospace', fontSize:'14px' }}>
            {loading ? '作成中...' : '▶ 冒険を始める'}
          </button>
        </form>
      </div>
