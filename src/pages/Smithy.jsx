...
export default function Smithy() {
  ...
  const doEnhance = async (item, source = 'equip', bookName = null, useCrystalOpt = false) => {
    ...
    // 天井（匠の祝福）：全プレイヤー。装備ごとに失敗でたまり、規定数で確定成功。
    ...
    const pityReady = blessingCap !== undefined && curBlessing >= blessingCap
    ...
    if (success) {
      const upd = { enhance_plus: nextPlus, blessing_count: 0 }  // 成功でリセット
    ...
    <div style={{ minHeight:'100vh', background:'#000820', padding:'16px', fontFamily:'monospace' }}>
      ...
      <div style={{ maxWidth:'700px', margin:'0 auto' }}>
        ...
        {selectedItem && (() => {
          ...
          const blessingCap = BLESSING_CAP[nextPlus]
          const pityReady = blessingCap !== undefined && blessing >= blessingCap
          // 匠の秘伝書（成功率アップ・全プレイヤー）
                ...
                {enhanceResult ? (
                  ...
                  <>
                    ...
                    <div style={{ textAlign:'center', marginBottom:'12px' }}>
                      <span style={{ fontSize:'9px', padding:'1px 4px', color: RARITY_COLORS[displayRarity(item)], border:`1px solid ${RARITY_COLORS[displayRarity(item)]}`, marginRight:'6px' }}>{RARITY_LABELS[displayRarity(item)]}</span>
                      <span style={{ color: RARITY_COLORS[displayRarity(item)], fontSize:'14px' }}>{w.name}{isShinka(item)?'★':''}{plus > 0 ? ` +${plus}` : ''}</span>
                    ...
                    <div style={{ fontSize:'10px', color:'#446688', marginBottom: (blessingCap !== undefined) ? '4px' : '12px' }}>
                    ...
                    return (
                      <div key={item.id} style={{ border:'1px solid #002244', background:'#001028', padding:'10px', marginBottom:'6px', opacity: isArtifactBase ? 0.5 : 1 }}>
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'6px' }}>
                          <div style={{ display:'flex', gap:'6px', alignItems:'center' }}>
                            <span style={{ fontSize:'9px', padding:'1px 4px', color:RARITY_COLORS[displayRarity(item)], border:`1px solid ${RARITY_COLORS[displayRarity(item)]}` }}>{RARITY_LABELS[displayRarity(item)]}</span>
                            <span style={{ color:RARITY_COLORS[displayRarity(item)], fontSize:'12px' }}>{w.name}{isShinka(item)?'★':''}{(item.enhance_plus||0)>0?` +${item.enhance_plus}`:''}</span>
                            {item.equipped && <span style={{color:'#0088ff',fontSize:'10px'}}>装備中</span>}
                  ...
                  <div key={item.id} style={{ border:`1px solid ${maxed ? '#ffcc00' : '#003a55'}`, background: maxed ? '#1a1400' : '#001028', padding:'10px', marginBottom:'6px' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'6px', gap:'8px', flexWrap:'wrap' }}>
                      <div style={{ display:'flex', gap:'6px', alignItems:'center', flexWrap:'wrap' }}>
                        <span style={{ fontSize:'9px', padding:'1px 4px', color: maxed ? '#ffcc00' : RARITY_COLORS[w.rarity], border:`1px solid ${maxed ? '#ffcc00' : RARITY_COLORS[w.rarity]}` }}>{maxed ? 'S' : RARITY_LABELS[w.rarity]}</span>
                        <span style={{ color: maxed ? '#ffcc00' : RARITY_COLORS[w.rarity], fontSize:'12px' }}>{w.name}{maxed?'★':''}{(item.enhance_plus||0)>0?` +${item.enhance_plus}`:''}</span>
                        {item.equipped && <span style={{ color:'#0088ff', fontSize:'10px' }}>装備中</span>}
                      </div>
                      {/* 段階ゲージ */}
