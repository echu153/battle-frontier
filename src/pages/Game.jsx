...
// 再修練5回でクラスLVキャップ解放：初期職は300・上位職のみ500
...
export const getEffectiveCap = (className, retraining) => {
  ...
  // 再修練5回で上限解放：初期職は300・上位職のみ500（2026-07-24）
  if (cnt >= 5) return INITIAL_CLASSES.includes(className) ? 300 : 500
...
export const executeSkill = (skill, eff, profile, enemy, enemyBuffs, playerBuffs, isArtifact, prevSkill = '') => {
  ...
  switch (skill.name) {
    ...
    case 'ソウルドレイン': {
      ...
      // 与えたダメージ(クリティカル・軽減込み)の30%を回復。上限は各エンジンで基本 最大HP20%/クリティカル時35%。
      // 実回復は各バトルエンジンの外側ループで finalDmg から算出する（=クリティカルが反映される）。
      result.drainRate = 0.3
      result.log = `💀 ソウルドレイン！ ${enemy.name}に${result.dmg}の特殊ダメージ！ 与えたダメージの30%を回復！`; break
    }
    ...
    case 'ルミナ': {
      ...
      if (tier === 0) {
        ...
        result.dmg = Math.floor(eff.matk*(2.0+(rt>=5?0.2:0))*am)
        // 与えたダメージ(クリティカル・軽減込み)の半分を回復。上限は各エンジンで基本 最大HP20%/クリティカル時35%。
...
export default function Game() {
  ...
  const refreshTownNotices = async (p) => {
    ...
    // 初心者ビンゴ①②: 受け取れる報酬（達成済みマス／成立ライン本数）の合計件数（一般公開）
    {
      try {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      if (ap) {
        ...
        petCharm = await loadCharmBonus(ap)  // チャーム＋リボン（リボンは特殊能力のみ引き継ぎ）
      }
    } catch { /* ペット未導入時は無視 */ }
  ...
  const doBattle = async (e) => {
    ...
    const doPlayerAttack = (isExtra=false) => {
      ...
      if (expandedSkillSet.length > 0) {
        ...
        if (cs && cs.skills && !BREEDER_PET_SKILLS.has(cs.skills.name) && playerMp >= mpCost) {
          ...
          if (playerAttacking && playerBuffs.bloodRage?.turns > 0 && finalDmg > 0 && !(playerBuffs.healSeal?.turns > 0)) {
            const rageCure = Math.min(Math.floor(finalDmg * playerBuffs.bloodRage.healRate), Math.floor(maxHp * healCapPct))
          ...
          if (res.drainRate > 0 && finalDmg > 0 && !(playerBuffs.healSeal?.turns > 0)) {
            const drainHeal = Math.min(Math.floor(finalDmg * res.drainRate), Math.floor(maxHp * healCapPct))
      ...
      if (!skillUsed) {
        ...
        if (playerBuffs.bloodRage?.turns > 0 && finalDmg > 0 && !(playerBuffs.healSeal?.turns > 0)) {
          const rageCure = Math.min(Math.floor(finalDmg * playerBuffs.bloodRage.healRate), Math.floor(maxHp * (isCrit ? 0.35 : 0.20)))
          playerHp = Math.min(maxHp, playerHp + rageCure)
          logs.push({ text:`🩸 血の狂気で${rageCure}回復！`, color:'#ff4444' })
        }
    ...
    if (applied && win && !isPapiaEncounter) {
      try {
        ...
        if (box?.ok && (box.mystery > 0 || box.strange > 0)) {
          if (box.mystery > 0) logs.push({ text: `🎁 不思議な箱 を見つけた！`, color: '#ffcc44' })
          if (box.strange > 0) logs.push({ text: `🎁 奇妙な箱 を見つけた！`, color: '#cc88ff' })
          setBattleLogs([...logs])
        }
      } catch { /* RPC未適用時は無視 */ }
    }
...
● 再修練を重ねるとスキルが段階的に強化、5回でそのクラスのLV上限が解放（初期職は300・上位職は500）`,
    },
    {
      id: 'm_shop', title: '🛒 商店',
...
● 各クラスのLV上限は100（再修練5回で上限解放：初期職は300・上位職は500）
...
● 再修練5回でそのクラスのLV上限が解放（初期職は300・上位職は500）
〔共通の効果〕
・他クラスでのステータス反映率+10%
    ...
    <div style={{ border:'1px solid #886600', background:'#001020', padding:'16px' }}>
      ...
      {isAtCap && retrainingCount < 5 && <div style={{ border:'1px solid #664400', background:'#0a0800', padding:'12px', marginBottom:'12px' }}>
        ...
        <div style={{ color:'#446688', fontSize:'10px', marginBottom:'8px', lineHeight:'1.6' }}>
          ...
          他クラスに転職後も各クラスの成長分は加算されます（上位職: 50%＋★×10%・最大100% ／ <span style={{color:'#ffcc00'}}>初期職: 50%＋★×30%（上限なし・★5で200%）</span>）。<br/>
          上限5回まで（★★★★★）。5回到達でこのクラスのレベル上限が解放されます（初期職は300・上位職は500）。
        </div>
      ...
      <div style={{ minHeight:'100vh', background:'#000820', fontFamily:'monospace' }}>
        ...
        {showMenu && (
          <div style={{ position:'fixed', top:'40px', right:'12px', background:'#001040', border:'1px solid #446688', zIndex:200, minWidth:'120px' }}>
            ...
            <button onClick={()=>{ nav('/status'); setShowMenu(false) }} style={{ display:'block', width:'100%', padding:'10px 16px', background:'none', border:'none', borderBottom:'1px solid #002244', color:'#aa88ff', cursor:'pointer', fontFamily:'monospace', fontSize:'12px', textAlign:'left' }}>📊 ステータス詳細</button>
            {profile?.is_admin && (
                    ...
                    <div style={{ border:'1px solid #003366', background:'#000a14', padding:'10px', marginTop:'8px' }}>
                      ...
                      {showChallengePanel && (
                        <div ref={challengePanelRef} style={{ border:'1px solid #8a3a44', background:'#160809', padding:'10px', marginTop:'8px' }}>
                          ...
                          {isTowerUnlocked(profile) && (
                            <button onClick={()=>{ nav('/tower'); setShowChallengePanel(false) }} style={{ width:'100%', padding:'12px', marginTop:'8px', background:'#0a1020', border:'1px solid #7fd4ff', color:'#7fd4ff', cursor:'pointer', fontFamily:'monospace', fontSize:'13px' }}>🗼 エンドレスタワー</button>
                          )}
                        </div>
                      )}
                      </MenuCat>
