...
function simulateAbyssBattle(eff, equipment, skillSets, profile, enemy, playerItem, floor = 1) {
  ...
  const doPlayerAttack = (isExtra = false) => {
    ...
    if (expandedSkillSet.length > 0) {
      ...
      if (isBreederCmd) {
        ...
        // ログは出所(紋章)を書かず、HPが動く事象だけ残す
        { const emDrain = emblemDrainAmount(eff, finalDmg, isPhysSkill); if (emDrain > 0 && !(playerBuffs.healSeal?.turns > 0)) { playerHp = Math.min(eff.hp_max, playerHp + emDrain); logs.push({ text:`💚 HPが${emDrain}回復した！`, color:'#44ff88' }) } }
        if (hasRokkan && pe('サイキッカー') && finalDmg > 0 && cs.skills?.type === '魔法攻撃') rokkanStacks = Math.min(6, rokkanStacks+1)
        // 効果は据え置き・発動ログのみ削除（装備の出所は表示しない方針）
        ...
        // 蒼雷の短刃: 追加行動の攻撃ヒット時、eff.extraParaChance%で相手を麻痺（発動ログは出さない）
    ...
    if (!skillUsed) {
      ...
      enemyHp -= finalDmg
      // 紋章: 物理/特殊吸収（ログは出所を書かずHPの増減だけ残す）
      { const emDrain = emblemDrainAmount(eff, finalDmg, !isMagical); if (emDrain > 0 && !(playerBuffs.healSeal?.turns > 0)) { playerHp = Math.min(eff.hp_max, playerHp + emDrain); logs.push({ text:`💚 HPが${emDrain}回復した！`, color:'#44ff88' }) } }
      // 効果は据え置き・発動ログのみ削除
      if (finalDmg > 0 && equippedWeaponItem?.bonus_effect === 'hit_heal_down_10_2t' && !(enemyBuffs.healDown?.turns > 0)) {
        enemyBuffs.healDown = { turns: 2, rate: 0.7 }
      }
  ...
  const doEnemyAttack = (isExtra = false) => {
    ...
    const pMdef = eff.mdef * (playerBuffs.mdefUp?.turns > 0 ? playerBuffs.mdefUp.rate : 1) * (playerBuffs.defUp?.turns > 0 ? playerBuffs.defUp.rate : 1) * (playerBuffs.mdefDown ? playerBuffs.mdefDown.rate : 1) * holyFieldDefE * holyKnightMultE * kabeDefE * ryurinMult
    ...
    const finalDmg = Math.floor(baseDmg*(isCrit?1.5:1.0)*dmgReduceRate*berserkDmgRate*enemyDmgDownRate*(1-playerDefRankReduction)*gambleBodyMult*allinDebuffInMult*evoTakenMult(eff, !isEM, playerHp / eff.hp_max)*ryurinReduce()*(0.9+Math.random()*0.2))
    playerHp -= finalDmg
  ...
  const scaleDamageToPlayer = (raw, atkStat, useStat /* 'atk'|'matk'|'hybrid' */, isCrit) => {
    ...
    const dmg = Math.max(0, Math.floor(raw * defScale * critMult * (1 - rankRed) * dmgReduceRate * evoTakenMult(eff, useStat !== 'matk', playerHp / eff.hp_max) * (0.9 + Math.random()*0.2)))
  ...
  while (playerHp > 0 && enemyHp > 0 && turn <= turnCap) {
    ...
    // 哭雨の羽衣: 5ターンごとに状態異常無効バフを再獲得（既にバフがある場合は重複しない・付与ログは出さない）
