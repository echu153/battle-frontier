...
test('味方のスキル：技名とダメージが色分けされる', () => {
  assert.equal(
    mark('3ターン目: おれおれお の⚔ 体当たり！ ゴブリンに1234の物理ダメージ！'),
    '3ターン目: おれおれお の⚔ [skill:体当たり]！ ゴブリンに[damage:1234]の物理ダメージ！'
  )
})

test('敵のスキル：「〜の技名！」も同じように色分けされる', () => {
  assert.deepEqual(
    kinds('3ターン目: 牛頭の斧兵の斧撃！ あなたに12,345ダメージ… 💥クリティカル！（2回）'),
...
test('クリティカルは技名として拾わない', () => {
  assert.deepEqual(
    kinds('3ターン目: おれおれお の💥クリティカル！ 攻撃！ 鉄角の仔牛に980ダメージ！'),
...
test('色を付けても文字列は元のまま（欠けたり増えたりしない）', () => {
  const samples = [
    '3ターン目: おれおれお の🔥 ファイアボール！ しかしゴブリンに回避された！',
    '🌋 3ターン目: 溶岩竜の噴火！ あなたに5000ダメージ…（必中）',
    '💚 タワーの加護！ HPが350回復した！',
    '',
  ]
