...
// プレイヤー共通ステータス上限（過剰分は5%のみ適用）。あとで調整しやすいよう1箇所に集約。
// 上限適用後の総合力が概ね 1.5 万前後になる想定（敵 target=20000 に対して挑む側はやや不利）。
...
const PALACE_META = [
  { palace:1, name:'【白羊】ハマル', arch:'warrior', dmg:'phys',
    ...
    strategy:'防御バフで耐えつつ、防御は並なので高火力で早期決着',
  ...
  { palace:2, name:'【金牛】アルデバラン', arch:'tank', dmg:'phys',
    ...
    strategy:'出血・毒などの固定ダメージで削る／長期戦に備え回復を厚く',
  ...
  { palace:5, name:'【獅子】レグルス', arch:'monk', dmg:'phys',
    ...
    strategy:'状態無効かスタン耐性を積む／防御を高めて受けきる',
  ...
  { palace:7, name:'【天秤】エルゲルビ', arch:'balanced', dmg:'hybrid',
    ...
    strategy:'ステータスは平均化して挑む／低威力スキルで攻める',
  ...
  { palace:8, name:'【天蠍】アンタレス', arch:'swift', dmg:'phys',
    ...
    strategy:'定期的に毒を解除／命中を上げて多段攻撃で削る',
  ...
  { palace:9, name:'【蒼穹】アウストラリス', arch:'swift', dmg:'phys',
    ...
    strategy:'素早さを上げて追加攻撃をさせない／回避は捨てて防御に回す',
  ...
  { palace:11, name:'【宝瓶】サダルメリク', arch:'priest', dmg:'mag',
    ...
    strategy:'バフ解除・回復阻害が必須／攻撃は別々のスキルを使う',
  ...
  { palace:12, name:'【星海】アルレシャ', arch:'arcane', dmg:'hybrid',
    ...
    strategy:'相手に都合の悪いバフは使わない／攻撃なら防御、特殊なら特防を上げる',
...
export const TENKYUU_PALACES = PALACE_META.map(m => ({
  ...
  mods: m.mods || {},
  title: m.title,
  enemy: { ...makeEnemy(m.name, TARGET, m.arch, m.dmg, m.statTweak), kit: m.kit || null, mods: m.mods || {} },
}))
