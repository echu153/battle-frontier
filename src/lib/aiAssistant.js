...
const GREETING_TRIGGER = /^(こんにちは|こんばんは|おはよ|おはようございます|やあ|よお|よっ|はじめまして|ハロー|hello|hi|hey|へい|ヘイ|どうも)/i
const THANKS_TRIGGER = /ありがと|あざ(す|っす)|サンキュー|感謝|thank|thx/i
const WHOAMI_TRIGGER = /(あなた|きみ|君|お前|おまえ|あんた|おまえさん)(って|は)?(誰|だれ|何者|なにもの|何なの|なになの)|何者|自己紹介|あなたについて|aiなの|ボットなの/i
const HELP_TRIGGER = /何ができ|なにができ|できること|できる事|使い方|つかいかた|ヘルプ|help|何が聞け|なにが聞け|何を聞け|なにを聞け|どんなこと(が|を)?(聞|質問|きけ|分か)|機能|メニュー|何を質問/
const LIMIT_TRIGGER = /答えられない|こたえられない|答えれない|こたえれない|答えら?んない|苦手|わからないこと|分からないこと|何がわから|なにがわから|できないこと|限界|何は答え/
...
export const askAssistant = async (query, ctx = {}) => {
  ...
  if (cls) {
    try {
      if (/スキル|わざ|技|アビリティ|何ができ|なにができ|できること/.test(raw)) {
