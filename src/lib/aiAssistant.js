...
・ステ振りはプロフィール等(1ptでHP+10/MP+5/他+1・転職しても減衰しない)。武器熟練度は出撃で上がり固定ボーナスに倍率。再修練(現クラス限定)5回でクラスLV上限が100→500。キャラLV100未満は獲得EXP1.5倍。
...
const classInfoText = (c) => {
  ...
  return `🛐 ${c}（${classType(c)}）\n${role}\n【転職条件】${how}\nクラスごとにLVは別管理。転職後に再修練を5回行うとLV上限が100→500に解放される。`
}
...
export const buildProgressionAdvice = (ctx) => {
  ...
  // 6) スキル/再修練
  steps.push('・スキル画面で習得スキルを確認。本職を極めたら神殿で「再修練」5回でスキル強化＆そのクラスのLV上限が100→500に解放')
  // 7) コンテンツでの素材集め
