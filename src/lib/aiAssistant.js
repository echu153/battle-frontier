...
export const KB = [
  ...
  { id: 'alchemy', keywords: ['錬金', '錬金部屋'],
    a: `🧪 錬金部屋（LV10で解放）は時間経過で強化石を自動生成(最大4枠)。放置で素材が貯まります。` },
  ...
  { id: 'scarecrow', keywords: ['かかし', '修練場', '熟練度'],
    a: `🌾 かかし修練場（LV10で解放）で武器の熟練度を安全に上げられます。熟練度が上がると固定ボーナスに倍率がかかり実効ステUP。` },
  ...
  { id: 'raid', keywords: ['レイド', 'レイドボス', 'あまざ', 'ヴァルゼノク'],
    a: `⚔ レイドボス（LV30で解放）は毎日21時/22時出現の高HPボス。与ダメに応じて報酬(EXP/素材/強化石)。素材は交換所で専用装備等と交換。日替わりでボス交替。` },
  { id: 'dungeon', keywords: ['ダンジョン', 'ローグライク', 'もぐる'],
    a: `🕳 ダンジョン（LV10で解放）はペットで挑むローグライク。奥ほど報酬増だが死亡で持ち物半分ロスト。倉庫預けは安全。撤退も大事。` },
  { id: 'pets', keywords: ['ペット', '懐き', '進化', 'チャーム'],
    a: `🐾 ペット（LV10で解放）は育てて懐かせ、進化やチャーム強化が可能。ダンジョンのお供。スキンシップや出撃で懐き度UP。` },
  { id: 'abyss', keywords: ['奈落', '闘技場', '挑戦'],
    a: `⚔ 奈落闘技場（LV30で解放）は20階の順番制。週1挑戦でGold・強化石・宝石が報酬。総合力を上げてから。` },
  { id: 'casino', keywords: ['賭博', '賭博場', 'カジノ', 'スロット'],
    a: `🎰 賭博場（LV10で解放）はATを使ってスロット等で遊べ、景品と交換可。あくまで運。使いすぎ注意。` },
  { id: 'fishing', keywords: ['釣り', '釣り場'],
    a: `🎣 釣り場（LV5で解放）で素材やアイテムが入手可。放置気味でも稼げるサブコンテンツ。` },
  { id: 'exchange', keywords: ['交換所', '交換'],
    a: `🔄 交換所（LV5で解放）でレイド素材等を装備・強化石・宝石と交換。集めた素材の使い道はここ。` },
  { id: 'museum', keywords: ['博物館', '寄贈', 'コレクション'],
    a: `🏛 博物館（LV5で解放）でアイテム寄贈しコレクションを埋め報酬獲得。ダブり品の使い道に。` },
...
export const buildProgressionAdvice = (ctx) => {
  ...
  // 3) 装備
  steps.push('・装備を整える：商店/ドロップで強い武器防具を入手 → 鍛冶屋で強化石を使って+強化（錬金部屋＝キャラLV10で解放＝で強化石を自動生成）')
  // 4) 熟練度
  if (lv >= 10) steps.push('・かかし修練場で使用武器の熟練度を上げる＝固定ボーナスに倍率がかかり実効ステUP')
  // 5) 宝石
  steps.push('・装飾品や装備に宝石を埋める（攻撃系=武器、防御系=防具、％系=装飾品）。交換所・博物館報酬で入手')
  // 6) スキル/再修練
  steps.push('・スキル画面で習得スキルを確認。本職を極めたら神殿で「再修練」5回でスキル強化＆キャラLV上限300解放')
  // 7) コンテンツでの素材集め
