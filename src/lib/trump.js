...
export const GAME_HELP = {
  daifugo: {
    name: '大富豪',
    basic: [
      '場に出ているより強い札を同じ枚数で出す。出せない・出したくないときはパス。',
      '強さは 3<4<…<K<A<2<JOKER。JOKERは1枚で最強＋どの組にも混ぜられる。',
      '同じ数字を2〜4枚まとめて出せる（場が2枚なら2枚で返す）。',
      '手札を先に無くした順に1位・2位…となり、最後まで残った人が最下位。',
    ],
    special: {
      kakumei: ['革命', '4枚以上を同時に出すと強さが逆転する（3が最強・2が最弱に）。もう一度4枚出すと元に戻る。'],
      spade3: ['スペ3返し', '場がJOKER1枚だけのとき、♠3の1枚で返せる。場が流れて自分がもう一度出せる。'],
      kaidan: ['階段', '同じマークの3枚以上の連番を出せる（例: ♠3♠4♠5）。JOKERは穴埋めに使える。4枚以上の階段は革命。'],
      shibari: ['しばり', '前の人と同じマークの組で返すとロックが発生し、場が流れるまでそのマークしか出せない（JOKERは自由）。'],
      miyako: ['都落ち', '前の対局の1位が今回1位を取れなかった瞬間、その場で最下位が確定する（2戦目以降）。'],
    },
    always: ['8切り', '8を含めて出すと場が流れ、出した人がもう一度好きな札を出せる。'],
    match: 'マッチ戦: 各局の順位でポイント（1位4pt・2位2pt・3位1pt・4位0pt）。合計ptで最終順位を決めます。',
  },
  speed: {
    name: 'スピード',
    basic: [
