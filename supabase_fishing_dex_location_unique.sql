...
--
-- 対応: ①旧「カンパチ@カリブ海」行を整理 → ②一意制約を (player_id, location, fish_name) へ差し替え
--       → ③衝突で登録できなかったプレイヤーへ「日本海のカンパチ」を補填
--
...
-- ============================================================
-- ① 影響を受けたプレイヤーを先に控える（このあと旧行を消すため）
-- ============================================================
CREATE TEMP TABLE _kanpachi_blocked ON COMMIT DROP AS
...
-- 救済で カリブカンパチ が既に入っている人は、旧「カンパチ@カリブ海」行を破棄
...
SELECT b.player_id, 'カンパチ', 'b', '日本海', now(), false
  FROM _kanpachi_blocked b
 WHERE EXISTS (
         SELECT 1 FROM public.fishing_records r
          WHERE r.player_id = b.player_id AND r.location = '日本海'
       )
   AND NOT EXISTS (
         SELECT 1 FROM public.fishing_records r
          WHERE r.player_id = b.player_id AND r.location = '日本海' AND r.fish_name = 'カンパチ'
       );
