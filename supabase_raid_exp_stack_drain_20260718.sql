...
-- ============================================================
-- 2026-07-18 【レイドEXPスタックを出撃(通常レベルアップ)でも反映】
--   ・supabase_anticheat_softclamp_20260710.sql の apply_battle_result をそのまま踏襲し、
--     「上限未満・非凍結でEXPを得たとき、貯めたレイドEXPスタック(raid_exp_stack)を
--       今回のEXPに合算してレベルアップ計算し、スタックを0にする」処理のみ追加。
--   ・これで出撃でレベルアップした瞬間に、レイド上限中に貯めたEXPが自動反映される。
--   ・前提: supabase_raid_exp_stack_20260718.sql（raid_exp_stack列・attack_raid_boss）適用済み。
--   ★ apply_battle_result はこのファイルが最新の正。protect_stats/scarecrow 等を再適用したら
--     anticheat_softclamp ではなく「このファイル」を最後に流すこと（署名は10引数で同一＝置換される）。
-- ------------------------------------------------------------
-- 以下、土台(2026-07-10 anticheat_softclamp)のヘッダ原文 --------
...
DECLARE
  ...
  v_stack_drained integer := 0;  -- ★レイドEXPスタックから今回反映した量
...
BEGIN
  ...
  -- ★上限未満・非凍結でEXPを得られる時は、貯めたレイドEXPスタックを合算してレベルアップ計算（後で自動反映）
  ...
  RETURN json_build_object('ok',true,'level_ups',v_level_ups,'new_lv',v_new_lv,
    ...
    'raid_stack_drained',v_stack_drained);
END;
$function$;
