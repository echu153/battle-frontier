...
--
-- 【流す順番】上から ①→②→③→④。②③は確認用（データを一切変えない）。
--   Supabaseのエディタは1文ずつ実行されるので、1文ずつコピーして流すこと。
--   ②がすべて0なら、そのまま④まで進んでよい。
--
-- 【前提・注意】
...
-- ①-2 バックアップ表をAPIから触れないように塞ぐ（publicの新規テーブルは既定でanon/authenticatedに
--      権限が付き、RLS無しだと誰でも読み書きできてしまうため）。必ず①の直後に実行する。
ALTER TABLE public.museum_backup_20260808 ENABLE ROW LEVEL SECURITY;

-- ①-3 同上（ポリシーを作らないので、この表に触れるのは service_role だけになる）
REVOKE ALL ON TABLE public.museum_backup_20260808 FROM anon, authenticated;


-- ============================================================
-- ② 事前チェック：再計算で取りこぼす記録が無いかを数える
--    ★すべて 0 なら安心して④へ進んでよい。0でない行があれば内容を見てから判断する。
-- ============================================================
...
-- ============================================================
-- ③ 変化プレビュー：誰がどれだけ増減するか（データは変えない）
--    powerは総合力への寄与（攻+防+特攻+特防+速+HP/10+MP/5）
-- ============================================================
WITH
...
-- 寄贈1件ごとの新しいボーナス量。
-- グループは「今日の装備リスト」で判定し、リストに無い装備だけ記録時の area_group を使う
...
-- ============================================================
-- ④ 本体：museum_* を再計算して上書き（★ここだけがデータを変える）
-- ============================================================
WITH
-- 装備名 → 所属グループ（Museum.jsx ITEM_GROUP_MAP と一致。ボス枠は 'boss'）
item_group(name, gid) AS (VALUES
...
-- ============================================================
-- UPDATE public.profiles p
--    SET museum_atk = b.museum_atk, museum_def = b.museum_def, museum_matk = b.museum_matk,
--        museum_mdef = b.museum_mdef, museum_spd = b.museum_spd,
--        museum_hp = b.museum_hp, museum_mp = b.museum_mp
