...
W('|---|---|---|---|')
{
  ...
  W('| 3 | ✅**修正済** 仮想ボスのHPを「同じ戦闘力のキャラのHP×3」に置き直した（旧：ユニークボスの式で本人の90倍あり、どの職も30秒で全滅して測定にならなかった） | 1挑戦で中央値47%まで削れるようになった | ATBのかかし |')
  W(`| 4 | ✅**修正済** 技に「威力が乗るステ（src）」を持たせて、職業補正の main と噛み合わせた | 5枠平均の開きが **3.95倍 → ${(a5[a5.length - 1] / a5[0]).toFixed(2)}倍** | 全戦闘 |`)
  W('| 5 | 同じ発動率の帯の中に残っている外れ値（③参照） | 1と4を直して大幅に減った | 全戦闘 |')
  W('| 6 | **未対応** バフの重ねがけに上限が無い（applyBuff は下限-90%だけ） | ATBの180秒でステが×6.4〜×8.0まで伸びる（実測：狩人 AGI 3,200→25,600） | ATB・長期戦 |')
}
W('')
W('### 1の中身（クリティカル率とLUK）')
W('')
...
W('**main が STR/INT 以外の職は、倍率が乗るステを伸ばせなかった**（いまは技の `src` で合わせてある）。')
W('')
