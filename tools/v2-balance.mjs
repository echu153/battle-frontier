...
W('|---|---|---|---|')
{
  ...
  W(`| 1 | **クリティカル率が LUK「差」の絶対値で決まる**（5% ＋ LUK差/100 × 10%・上限50%）。戦闘力が上がるとLUK差が数千になり、**クリ率は1%か50%かの二択**になる | 壁(LUK${Math.round(wallStats().luk).toLocaleString()})に対して LUK${lo.luk}→クリ${(lo.crit * 100).toFixed(1)}%・平均${Math.round(lo.avg).toLocaleString()} ／ LUK${hi.luk}→クリ${(hi.crit * 100).toFixed(1)}%・平均${Math.round(hi.avg).toLocaleString()}（**+${Math.round((hi.avg / lo.avg - 1) * 100)}%**） | 全戦闘 |`)
  W(`| 2 | **出血・毒が「最大HPの%」ダメージ**なので、HPが巨大なユニークボスほど直接攻撃より効く | 戦闘力${POWER.toLocaleString()}のボス(HP${bossHp.toLocaleString()})で 毒1刻み${poisonTick.toLocaleString()}／出血5スタック${bleedTick.toLocaleString()}。直接攻撃の主力は7,000〜13,000 | ATB・ユニークボス・レイド |`)
...
W('')
W('LUK差450でクリ率が1%→50%（上限）に張り付く。**サブがLUKの職（賢者・異端審問官）が突出しているのはこれが理由**。')
W('')
W('### 4の中身（参照ステと職業補正のズレ）')
W('')
