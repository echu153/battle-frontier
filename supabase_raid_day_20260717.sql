...
--
--  【昼】12〜17時のうち毎日ランダムな1枠に1体だけ出現（30分・HP400万）。
...
-- 4) 枠ごとのHP（昼は人が少ない時間帯なので低め。2026-07-18: 200万→400万）
CREATE OR REPLACE FUNCTION raid_slot_hp(p_slot int)
RETURNS int LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE WHEN p_slot IN (21, 22) THEN 7000000 ELSE 4000000 END
$$;
...
-- 8) 管理者テスト用: 昼枠の条件（HP400万・順位報酬なし）でも試せるよう slot を指定できるようにする
